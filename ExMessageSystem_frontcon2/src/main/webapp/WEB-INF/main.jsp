...
s
				<!-- Menu -->
					<nav id="menu">	
						...
						<ul class="actions vertical">
							<li><h5>회원가입</h5></li>
								<!-- 
									action : JoinService 서블릿 도착!
									method : post
									name : email, pw, tel, address
								-->
						...
						<div class="inner">
							<section>
								...
								<div class="field half first">
									...
									<ul class="actions">
										<li><input id ="send" disabled="disabled" type="submit" value="Send Message" class="special" /></li>
		...
		<script type="text/javascript">
			//1. 필요한 태그들을 가져오기 (input, button 총 2개 핗요험)
			let email = document.getElementById("email");
			let sned = document.getElementById("send");

			//2. input 태그에 키보드에 관한 이벤트 처리하기
			email.addEventListener("keyup", ()=>{
				// 3. 키보드를 누를 때마다 console 창에 input 태그 값을 출력해주기	
				let sendData = email.value;
				console.log("입력값: ",sendData);
				//4. fetch 통신을 사용하여 sendData보내주기
				// url : CheckEmail.do
				let url = "CheckEmail.do"
					fetch(url + "?sendData=" + sendData)
					  .then((res) => {
					    return res.json();
					  })
					  .then((result) => {
